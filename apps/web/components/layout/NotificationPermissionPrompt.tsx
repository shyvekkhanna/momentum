...
import { webStorageDriver } from "@/lib/storage/webStorageDriver";
...
export function NotificationPermissionPrompt() {
  ...
  useEffect(() => {
    ...
    webStorageDriver.getItem(DISMISS_KEY).then((dismissed) => {
      if (cancelled || dismissed) return;
      if (webNotificationService.getPermissionState() === "default") {
  ...
  const dismiss = () => {
    webStorageDriver.setItem(DISMISS_KEY, "1");
    setVisible(false);
  };
